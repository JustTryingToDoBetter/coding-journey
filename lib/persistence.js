...
const BOARD_COLUMNS = ['backlog', 'this_week', 'in_progress', 'blocked', 'completed'];
...
function createDefaultStore() {
  return {
    ...
    settings: {
      ...
      evidenceFilters: { track: 'all', type: 'all', search: '' },
      boardCapacity: { maxTasks: 10, maxHours: 9 },
    },
  };
}

function createDemoBoard() {
  const activeSeedIds = ROADMAP_DATA.tasks
    .filter((task) => task.weekNumber === ROADMAP_DATA.roadmap.currentWeek)
    .slice(0, 12)
    .map((task) => task.id);
  const tasksById = ROADMAP_DATA.tasks.reduce((acc, task) => {
    const index = activeSeedIds.indexOf(task.id);
    let column = task.isActiveThisWeek ? 'this_week' : 'backlog';
    if (index >= 4 && index < 7) column = 'in_progress';
    else if (index >= 7 && index < 9) column = 'blocked';
    else if (index >= 9 && index < 11) column = 'completed';
    acc[task.id] = { column };
    return acc;
  }, {});
  return { tasksById };
...
function migrateStore(parsed) {
  const normalized = {
    ...
    settings: {
      ...
      boardCapacity: {
        maxTasks: parsed?.settings?.boardCapacity?.maxTasks || 10,
        maxHours: parsed?.settings?.boardCapacity?.maxHours || 9,
      },
    },
  ...
  ROADMAP_DATA.tasks.forEach((task) => {
    ...
    const column = BOARD_COLUMNS.includes(existingColumn)
      ? existingColumn
      : status === 'not_started'
        ? (task.isActiveThisWeek ? 'this_week' : 'backlog')
        : (STATUS_TO_COLUMN[status] || 'backlog');
...
function setTaskStatus(store, taskId, status) {
  ...
  setBoardTaskColumn(store, taskId, STATUS_TO_COLUMN[status] || 'backlog');
}

function getTaskBoardColumn(store, taskId) {
  const column = store.board?.tasksById?.[taskId]?.column;
  return BOARD_COLUMNS.includes(column) ? column : 'backlog';
}
...
function clearWeeklyPlan(store) {
  ROADMAP_DATA.tasks.forEach((task) => {
    const column = task.isActiveThisWeek ? 'this_week' : 'backlog';
    setBoardTaskColumn(store, task.id, column);
    if (getTaskState(store, task.id).status === 'not_started') return;
    if (column === 'backlog' || column === 'this_week') {
      store.taskState[task.id] = { ...getTaskState(store, task.id), status: 'not_started' };
    }
  });
}

function resetAllProgress(store) {
  store.taskState = {};
  store.evidence = {};
  store.reviews = {};
