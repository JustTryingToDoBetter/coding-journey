...
function getWeekStats(week, getTaskState, getEvidence) {
  ...
  const shipped = tasks.reduce((sum, task) => sum + (getTaskState(task.id).status === 'shipped' ? 1 : 0), 0);
...
function getDashboardMetrics(tasks, weeks, getTaskState, getEvidence) {
  ...
  const shippedTasks = normalizedTasks.reduce((sum, task) => sum + (getTaskState(task.id).status === 'shipped' ? 1 : 0), 0);
  ...
  return {
    ...
    buildVsTheory: `${normalizedTasks.filter((t) => t.kind === 'project').length} / ${normalizedTasks.filter((t) => t.kind === 'theory').length}`,
    tracks,
    pctLabel: formatPct(completionPct),
    totalEvidenceEntries: countEvidence(normalizedTasks.reduce((acc, task) => {
