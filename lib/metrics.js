...
function getBoardMetrics(boardTasks, boardCapacity = { maxTasks: 10, maxHours: 9 }) {
  const todayIso = new Date().toISOString().slice(0, 10);
  const weeklyTasks = boardTasks.filter((task) => task.isActiveThisWeek);
  const weeklyCompleted = weeklyTasks.filter((task) => task.column === 'completed');
  const activeTasks = boardTasks.filter((task) => task.column === 'this_week' || task.column === 'in_progress');
  const blockedTasks = boardTasks.filter((task) => task.column === 'blocked');
  const overdueTasks = boardTasks.filter((task) => task.column !== 'completed' && task.dueDate && task.dueDate < todayIso);
  return {
    weeklyCompletedTasks: weeklyCompleted.length,
    weeklyCompletedHours: Math.round(weeklyCompleted.reduce((sum, task) => sum + (task.durationHours || 0), 0)),
    activeWorkloadHours: Math.round(activeTasks.reduce((sum, task) => sum + (task.durationHours || 0), 0)),
    blockedCount: blockedTasks.length,
    overdueCount: overdueTasks.length,
    thisWeekCount: boardTasks.filter((task) => task.column === 'this_week').length,
    thisWeekHours: Math.round(
      boardTasks.filter((task) => task.column === 'this_week').reduce((sum, task) => sum + (task.durationHours || 0), 0)
    ),
    overCapacity:
      boardTasks.filter((task) => task.column === 'this_week').length > boardCapacity.maxTasks ||
      boardTasks.filter((task) => task.column === 'this_week').reduce((sum, task) => sum + (task.durationHours || 0), 0) >
        boardCapacity.maxHours,
  };
}
