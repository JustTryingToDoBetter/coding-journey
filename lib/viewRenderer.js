...
export function renderMain({
  ...
  return `<div class="app-shell">
    ...
    ${renderWeeklyBoard(boardTasks, boardMetrics, boardCapacity)}
    ${renderPhaseView(phase, visibleMilestones, getMilestoneStats, getTaskState)}
    <section id="deferred-insights" class="panel panel--glass" aria-live="polite" aria-busy="true">
      <p class="helper-text">Loading insights…</p>
    </section>
  </div>`;
}

export function renderDeferredInsights({ getEvidence, evidenceFilters, metrics, getSkillScores, getTaskState, getSeniorManagerReadiness, getMilestoneStats, getNext90DayPlan }) {
